  ...
  <div class="player" v-show="playList.length>0">
    ...
    <transition name="mini">
      <div class="mini-player" v-show="!fullScreen" @click="open">
        ...
        <div class="control">
          <PregressCircle :radius="radius" :percent="percent">
    ...
    <audio ref="audio" :src="currentSong.url" @error="error" @play="canplay" @timeupdate="updatetime" @ended="end"></audio>
  </div>
</template>
...
import PregressCircle from 'base/progress-circle/progress-circle'
  ...
  methods:{
    ...
    middleTouchMove(e){
      if (this.touch.init) {
        ...
        if (Math.abs(deltaX)>Math.abs(deltaY)) { //只支持横向滚动 横轴位移大于纵轴 说明要滑屏 否则是滚动歌词
          const left = this.currentShow === 'cd' ? 0 : -window.innerWidth //页面中线位置的偏移距离
          const offsetWidth = left+deltaX
          // const offsetWidth = Math.min(0, Math.max(-window.innerWidth, left + deltaX))
          this.touch.percent = Math.abs(offsetWidth/window.innerWidth) //移动百分比
    ...
    handleLyric({lineNum,txt}){
      ...
      if (lineNum > 5) { //dom数组的大于5的当前行滑动到自身 时间1s
        let lineEl = this.$refs.lyricLine[lineNum-5]
        this.$refs.lyricList.scrollToElement(lineEl,1000)
      }else{
        this.$refs.lyricList.scrollTo(0,0,1000) //dom数组的前5行滑动到0,0
      }
    },
    getLyric() {
  ...
  watch:{
    currentSong(newSong,oldSong){
      ...
      if (newSong.id && newSong.id !== oldSong.id) {
        ...
        // this.$nextTick(()=>{ //微信里切到后台会继续播放完但不会执行end
