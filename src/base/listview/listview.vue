<template>
  <Scroll class="listview" :data="data" ref="listview" :listenScroll="listenScroll" @scroll="scroll" :probeType="probeType">
    ...
    <div class="list-shortcut" @touchstart="onShortcutTouchStart" @touchmove.stop.prevent="onShortcutTouchMove">
      <ul>
          <li :class="{'current':currentIndex == index}" v-for="(item,index) in shortcutList" class="item" :index="index">
            {{item}}
          </li>
      </ul>
    </div>
...
export default {
  ...
  methods:{
    ...
    onShortcutTouchMove(e){
      ...
      let distance = Math.floor((this.touch.y2-this.touch.y1)/ANCHOR_HEIGHT)
    ...
    scrollY(newY,oldY){
      ...
      for(let i=0;i<listHeight.length-1;i++){
        ...
        if(-newY>=height1 && -newY<height2){
          ...
          this.diff = height2+newY //计算区间上限到滚动位置还剩的距离差 newY是负数
          return
        }
      }
      // 滚动到底部 -newY大于最后一个的区间上限
      this.currentIndex = listHeight.length-2
    },
    diff(newDiff){
      let fixedTop = (newDiff>0 && newDiff<TITLE_HEIGHT) ? newDiff-TITLE_HEIGHT : 0;
      // console.log(newDiff,fixedTop)
      if(this.fixedTop !== fixedTop){
        this.$refs.fixed.style.cssText = `transform:translate3d(0,${fixedTop}px,0)`
      }
    }
  },
  components:{
